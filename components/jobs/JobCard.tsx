      ...
      <Card hover className="h-full">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {job.skills.map((skill) => (
            <span
              key={skill}
              className="px-2 py-1 text-xs rounded-lg bg-brand-500/10 text-brand-300 border border-brand-500/20"
            >
              {skill}
            </span>
          ))}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-white/10">
          ...
          <div className="text-right">
            <p className="text-lg font-bold text-brand-300">{formatCurrency(job.budget, job.currency)}</p>
            <p className="text-xs text-white/40">{job.applicantCount} applicants</p>
          </div>
        </div>

        {job.createdAt && (
          <p className="text-xs text-white/30 mt-2">{formatDate(job.createdAt)}</p>
        )}
      </Card>
    </Link>
  );
