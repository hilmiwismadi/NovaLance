    ...
    <Link href={`/projects/${project.id}`}>
      <Card hover className="h-full">
        <div className="flex items-start justify-between mb-3">
          <h3 className="text-lg font-semibold text-white line-clamp-2 flex-1 mr-2">{project.title}</h3>
        ...
        <div className="space-y-3">
          ...
          <div>
            <div className="flex items-center justify-between text-sm mb-1">
              ...
              <span className="text-white/90">{completedMilestones}/{project.milestones.length} milestones</span>
            </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-white/10">
            <div>
              <p className="text-lg font-bold text-brand-300">{formatCurrency(project.totalBudget, project.currency)}</p>
            </div>
            {getRoleBadge()}
          </div>
          ...
          {project.userRole === 'owner' && project.freelancer && (
            <div className="flex items-center gap-2 text-sm">
              <span className="text-white/40">Freelancer:</span>
