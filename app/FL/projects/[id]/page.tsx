...
import { getFLProjectById } from '@/lib/mockData';
...
export default function FLProjectDetailPage() {
  ...
  if (!project) {
    ...
    const mockProject = getFLProjectById(projectId);
    if (mockProject) {
      return <MockFLProjectDetailPage project={mockProject} />;
    }
    return (
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-3 sm:px-4 md:px-6 space-y-4 sm:space-y-6">
      ...
      <Modal isOpen={submitModalOpen} onClose={() => setSubmitModalOpen(false)} title="Submit Milestone" size="lg">
        <div className="space-y-4">
          {selectedMilestone && (
            <>
              <p className="text-slate-600">
                Submit your work for milestone #{selectedMilestone.index + 1}. Provide details about your deliverables for the project owner to review.
              </p>
              ...
              <div>
                  ...
                  {links.map((link, index) => (
                    <div key={index} className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-3 py-2 rounded-lg border border-slate-300 focus:border-brand-500 focus:ring-2 focus:ring-brand-200 outline-none transition-all text-sm"
                      />
                      <Button
                        variant="ghost"
                        onClick={() => {
                ...
                <p className="text-xs text-slate-500 mt-2">
                  Add links to your deliverables such as GitHub repositories, Figma designs, live demos, etc.
                </p>
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                <p className="text-sm text-amber-800">
                  <strong>Note:</strong> Submitting indicates that you have completed the work for this milestone. The project owner will need to review and approve it before you can withdraw payment.
                </p>
              </div>
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  className="flex-1"
                  disabled={isSubmitPending || isSubmitConfirming}
                >
                  Cancel
                </Button>
                <Button
                  ...
                  className="flex-1"
                  disabled={isSubmitPending || isSubmitConfirming || !submissionDescription.trim()}
                >
...
// Mock FL project detail page component for when no contract data is available
interface MockFLProjectDetailPageProps {
  project: import('@/lib/mockData').FLProject;
}

function MockFLProjectDetailPage({ project }: MockFLProjectDetailPageProps) {
  const router = useRouter();
  const [submitModalOpen, setSubmitModalOpen] = useState(false);
  const [withdrawModalOpen, setWithdrawModalOpen] = useState(false);
  const [selectedMilestone, setSelectedMilestone] = useState<{ index: number; data: any } | null>(null);
  ...
  // Calculate progress
  const completedMilestones = project.milestones.filter(m => m.accepted || m.released).length;
  const totalMilestoneCount = project.milestones.length;
  const progress = totalMilestoneCount > 0 ? (completedMilestones / totalMilestoneCount) * 100 : 0;

  const getMilestoneStatus = (milestone: any) => {
    if (milestone.released) return { text: 'Withdrawn', variant: 'success' as const };
    if (milestone.accepted) return { text: 'Ready to Withdraw', variant: 'success' as const };
    if (milestone.submissionTime > BigInt(0)) return { text: 'Submitted - Pending Approval', variant: 'warning' as const };
  ...
  const canWithdraw = true; // In mock mode, allow withdrawal for demo
      ...
      <Card className="p-4 sm:p-6 bg-gradient-to-br from-slate-50 to-brand-50/30 border-brand-200/30">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div>
            <p className="text-[10px] sm:text-xs text-slate-500 uppercase tracking-wide font-medium">Total Budget</p>
            <p className="text-base sm:text-lg font-bold text-brand-600">
              {(project.totalBudget / 1e6).toLocaleString()} IDRX
            </p>
          </div>
          <div>
            <p className="text-[10px] sm:text-xs text-slate-500 uppercase tracking-wide font-medium">Milestones</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {project.milestones.map((milestone) => {
            const milestoneStatus = getMilestoneStatus(milestone);
            const isExpanded = expandedMilestones.has(milestone.index);
            const canSubmit = milestone.submissionTime === BigInt(0);
            const canWithdraw = milestone.accepted && !milestone.released;

            return (
              <Card
                key={milestone.id}
                className={`overflow-hidden ${(canSubmit || canWithdraw) ? 'cursor-pointer hover:shadow-md' : ''}`}
                onClick={() => (canSubmit || canWithdraw) && toggleMilestoneExpanded(milestone.index)}
              >
                <div className="p-4 sm:p-5 pb-3">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <h3 className="text-base sm:text-lg font-semibold text-slate-900">Milestone {milestone.index + 1}</h3>
                        <Badge variant={milestoneStatus.variant} className="text-xs flex-shrink-0">
                          {milestoneStatus.text}
                        </Badge>
                        {milestone.isLastMilestone && (
                          <Badge variant="default" className="text-xs flex-shrink-0">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <p className="text-xs text-slate-500">Amount</p>
                      <p className="text-base sm:text-lg font-bold text-brand-600">
                        {(Number(milestone.actualAmount) / 1e6).toLocaleString()} IDRX
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="border-t border-slate-100 p-4 sm:p-5 bg-slate-50/30">
                    <div className="space-y-3">
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-600">Percentage</span>
                        <span className="font-medium text-slate-900">{milestone.percentage / 100}%</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-600">Submitted</span>
                        <span className="font-medium text-slate-900">
                      ...
                      {milestone.yieldAmount > BigInt(0) && (
                        <div className="flex justify-between text-sm">
                          <span className="text-slate-600">Yield Amount</span>
                          <span className="font-medium text-emerald-600">{(Number(milestone.yieldAmount) / 1e6).toLocaleString()} IDRX</span>
                        </div>
                      )}
                      ...
                      {canSubmit && (
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setSelectedMilestone({ index: milestone.index, data: milestone });
                            setSubmitModalOpen(true);
                          }}
                        >
                          Submit Milestone
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-400 to-slate-600 flex items-center justify-center">
                <span className="text-sm font-bold text-white">
                  {project.creatorEns?.[0].toUpperCase() || project.creator.slice(0, 2).toUpperCase()}
                </span>
              </div>
              <div>
                <p className="text-sm font-medium text-slate-900">
                  {project.creatorEns || `${project.creator.slice(0, 6)}...${project.creator.slice(-4)}`}
                </p>
                <p className="text-xs text-slate-500">Creator</p>
              </div>
            </div>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">Total Deposited</span>
                <span className="font-medium text-slate-900">{(Number(project.totalDeposited) / 1e6).toLocaleString()} IDRX</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">Vault (90%)</span>
                <span className="font-medium text-brand-600">{(Number(project.vaultAmount) / 1e6).toLocaleString()} IDRX</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">Lending (10%)</span>
                <span className="font-medium text-blue-600">{(Number(project.lendingAmount) / 1e6).toLocaleString()} IDRX</span>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6 bg-blue-50 border-blue-200">
            ...
            <p className="text-sm text-slate-600 mb-4">
              This project is showing mock data. The real project data will be loaded from the ProjectLance contract.
            </p>
            <Button
              variant="outline"
              className="w-full border-blue-300 text-blue-700 hover:bg-blue-100"
              onClick={() => router.push('/FL/projects')}
            >
              Back to Projects
            </Button>
          </Card>
      ...
      <Modal isOpen={submitModalOpen} onClose={() => setSubmitModalOpen(false)} title="Submit Milestone" size="lg">
        <div className="space-y-4">
          {selectedMilestone && (
            <>
              <p className="text-slate-600">
                Submit your work for milestone "{selectedMilestone.data.name}". Provide details about your deliverables for the project owner to review.
              </p>
              ...
              <div className="bg-slate-50 rounded-lg p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Milestone:</span>
                  <span className="font-medium text-slate-900">{selectedMilestone.data.name}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Percentage:</span>
                  <span className="font-medium text-slate-900">{selectedMilestone.data.percentage / 100}%</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Deadline:</span>
                  <span className="font-medium text-slate-900">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Payment Amount:</span>
                  <span className="font-medium text-brand-600">{(Number(selectedMilestone.data.actualAmount) / 1e6).toLocaleString()} IDRX</span>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <Button
                  ...
                  className="flex-1"
                  disabled={!submissionDescription.trim()}
                >
                  Submit Progress
                </Button>
              </div>
            ...
            <>
              ...
              <div className="bg-slate-50 rounded-lg p-4 space-y-2">
                ...
                {selectedMilestone.data.isLastMilestone && selectedMilestone.data.yieldAmount > BigInt(0) && (
                  <>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-600">Yield Amount:</span>
                      <span className="font-medium text-emerald-600">{(Number(selectedMilestone.data.yieldAmount) / 1e6).toLocaleString()} IDRX</span>
                    </div>
                    <div className="border-t border-slate-200 pt-2 flex justify-between text-sm">
                      <span className="text-slate-600">Total to Receive:</span>
                      <span className="font-bold text-slate-900">
                        {((Number(selectedMilestone.data.actualAmount) + Number(selectedMilestone.data.yieldAmount)) / 1e6).toLocaleString()} IDRX
                      </span>
                    </div>
                  </>
                )}
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                <p className="text-sm text-emerald-800">
                  <strong>Mock Mode:</strong> In production, this will execute a blockchain transaction to withdraw your funds.
                </p>
              </div>

              <div className="flex gap-3">
                <Button variant="ghost" onClick={() => setWithdrawModalOpen(false)} className="flex-1">
                  Cancel
                </Button>
                <Button variant="success" onClick={() => setWithdrawModalOpen(false)} className="flex-1">
                  Withdraw (Mock)
                </Button>
              </div>
            </>
